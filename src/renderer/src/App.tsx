...
function AccountEditor({ account, onClose }: { account?: PublicAccount; onClose(): void }): JSX.Element {
  ...
  const submit = async (event: React.FormEvent): Promise<void> => {
    ...
    const input: AccountInput = { id: account?.id, accountMode: mode, label: value('label'), email: value('email'), accountId: value('accountId'), accessToken: value('accessToken') || undefined, apiEndpoint: value('apiEndpoint'), apiKey: value('apiKey') || undefined, fiveHourWeekPercent: Number(value('ratio') || 16) }
